...
# Modify model_points according to the chosen landmarks
...
def calculate_head_angle(eye_left, eye_right, nose_tip):
    ...
    return angle_deg
# Load the model
# Use the local weights file present in the project
    ...
    for detection in detections:
        if int(detection[5]) == 67:  # 67 is the index of 'cell phone'
            ...
            if COUNTER2 >= 3:
                ...
                cv2.putText(img, "Put away your CELL PHONE!", (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                sound_thread("phone")
                COUNTER2 = 0
    for face in faces:
            ...
            if len(landmarks_points) >= len(model_points):
                ...
                image_points = np.array([
                    landmarks_points[30],  # Tip of the nose
